...
if [ -f "/opt/RAGAssistant/chrome-sandbox" ]; then
    chmod 4755 "/opt/RAGAssistant/chrome-sandbox"
    echo "Set chrome-sandbox permissions"
fi

# Ensure the application is executable
if [ -f "/opt/RAGAssistant/rag-assistant" ]; then
    chmod +x "/opt/RAGAssistant/rag-assistant"
    echo "Made application executable"
fi

# Make the backend launcher wrapper executable
if [ -f "/opt/RAGAssistant/resources/start-backend.sh" ]; then
    chmod +x "/opt/RAGAssistant/resources/start-backend.sh"
...
DESKTOP_FILE="/usr/share/applications/rag-assistant.desktop"
if [ -f "$DESKTOP_FILE" ]; then
    ...
    if ! grep -q "disable-gpu-sandbox" "$DESKTOP_FILE"; then
        ...
        sed -i 's|^Exec=/opt/RAGAssistant/rag-assistant|Exec=/opt/RAGAssistant/rag-assistant --disable-gpu-sandbox|g' "$DESKTOP_FILE"
