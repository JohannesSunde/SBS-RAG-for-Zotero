...
export function registerChatTab() {
  ...
  ((window as any).Zotero as any).PaneManager.registerLibraryTabPanel({
...
function renderChatUI(container: HTMLElement) {
  ...
  header.textContent = "RAG Assistant for Zotero";
...
async function onAsk(question: string, container: HTMLElement) {
  ...
  try {
    ...
    // @ts-ignore
    const ids = (window as any).Zotero.getActiveZoteroPane().getSelectedItems().map(i => i.id).join(',');
    const resp = await chat(question, [ids]); // chat expects string[]
...
function renderResult(root: HTMLElement, resp: ChatResponse) {
  ...
  for (const c of resp.citations) {
    ...
    li.textContent = `${c.title} (${c.year ?? "n.d."}) [${c.id}]`;
